...
Notice that as the number of bins increases, the confience intervals become wider; however, this is not always the case. If the bins are chosen to decrease the standard deviation in each bucket enough, the confidence intervals will become narrower. Making a good calibration plot requires careful consideration of the number of bins and the method of choosing the bins.
...
plot_multinomial_calibration <- function(predicted_probs, actual_class, n_bins=10) {
  ...
  ggplot(plot_data, aes(x=bin_center, color=class, group=class)) +
    ...
    geom_point(aes(y=observed_prop, size = count)) +
